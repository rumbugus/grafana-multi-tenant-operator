...
def create(api, name, email, organizationNames):
    response = api.admin.create_user(
        {"name": name, "email": email, "login": name, "password": "changemeplease"}
    )
...
def update(api, userId, name, email, oldOrganizationNames, newOrganizationNames):
    response = api.users.update_user(
        userId, {"email": email, "name": name, "login": name}
    )
    revokeOrgs = [
        organization.find_by_name(api, orgName).get("id")
...
def _add_organizations(api, name, orgIds):
    _data = {"loginOrEmail": name, "role": "Viewer"}
    for orgId in orgIds:
