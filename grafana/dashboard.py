...
async def update(
    ...
    try:
        pruneOrgIds = [
            organization.find_by_name(api, orgName).get("id") for orgName in pruneOrgs
        ]
    except GrafanaException as err:
        logger.error(f"Unable to get organization by name in update dashboard: {err}")
    if pruneOrgIds:
        await lock.acquire()
        try:
    ...
    try:
        for orgId in orgIds:
            try:
                ...
                logger.error(f"Unable to switch to organization with id={orgId}: {err}")
            try:
                api.dashboard.delete_dashboard(oldName)
            except GrafanaException as err:
